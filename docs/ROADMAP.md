...
| **"Recent conversations isn't pulling anything"** | `collapseVisits` dropped any conversation whose stored history title was empty or the bare product name. That is the *common* case, not the edge case: these sites are single-page apps, so the address becomes `/c/<id>` the moment the first message is sent — before the model has named the chat — and Chrome never revises the title it recorded. Measured: history held four conversations, `chrome.history.search` returned them, and **zero rows rendered**. They are kept now, named from the prompt that started them where one binds, and from the provider otherwise |
| ...and clicking the row "did nothing" | Same silence as the toggles: `if (granted)` with no `else`. Chrome's popup for `history` is also the scariest one it has — "read and change your browsing history on all your signed-in devices", against a row promising "chatgpt.com only, on this device" — so declining is a *reasonable* reaction that then went unexplained. The row now says what Chrome will ask before the click, and what happened after a refusal |
